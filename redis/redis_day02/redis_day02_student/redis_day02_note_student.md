...
```python
1、节约内存空间   key field value   键--字段--值
...
# 6、返回多个字段值
HMGET key field filed                hmget key field filed
# 7、返回所有的键值对
HGETALL key                          hgetall key  字典
# 8、返回所有字段名
HKEYS key                            hkeys  key
# 9、返回所有值
HVALS key                            hvals  key
# 10、删除指定字段
HDEL key field                       hdel key field  删除
# 11、在字段对应值上进行整数增量运算
HINCRBY key filed increment          hincrby key filed increment
# 12、在字段对应值上进行浮点数增量运算
...
2、用户维度统计
   ...
   比如关注了5人
	 ...
	 HINCRBY user:10000 fans 1  hincrby user:10000 fans 1
```
...
```python
# 1、增加一个或者多个元素,自动去重
SADD key member1 member2                     sadd  key
# 2、查看集合中所有元素
SMEMBERS key                                 amembers  key
# 3、删除一个或者多个元素，元素不存在自动忽略
SREM key member1 member2                     srem key 
# 4、元素是否存在
SISMEMBER key member                         sismember  key 
# 5、随机返回集合中指定个数的元素，默认为1个
SRANDMEMBER key [count]                      srandmember key 
# 6、弹出成员
SPOP key [count]                             spop key
...
SDIFF key1 key2                               sdiff
# 10、差集保存到另一个集合中
SDIFFSTORE destination key1 key2              sdiffstore

# 11、交集  中间
SINTER key1 key2                              sinter 
SINTERSTORE destination key1 key2             sinterstore

# 11、并集
SUNION key1 key2                              sunion
SUNIONSTORE destination key1 key2             sunionstore
```
...
user001和user002的共同关注为:
	SINTER user001-002:inter user001 user002
	结果为: {'peiqi','qiaozhi'}
```
