...
| list       | 有序可重复列表                                               | ==关注列表，好友关注,粉丝列表，消息队列等==                  |
...
- **第一种redis数据持久化分类之 - RDB模式（默认开启）**     恢复数据库
...
**创建rdb文件的两种方式**        /etc  配置的文件      /var 产生的文件
...
# 执行过程如下
1、客户端 发送 BGSAVE 给服务器                                 bg后台
2、服务器马上返回 Background saving started 给客户端
3、服务器 fork() 子进程做这件事情                         fork   创建进程
4、服务器继续提供服务
5、子进程创建完RDB文件后再告知Redis服务器
显示行    :set number
# 配置文件相关操作
/etc/redis/redis.conf                                         重点
...
- **第二种redis数据持久化分类之 - AOF（AppendOnlyFile，默认未开启）**
...
```python
1、创建RDB文件需要将服务器所有的数据库的数据都保存起来，这是一个非常消耗资源和时间的操作，所以服务器需要隔一段时间才创建一个新的RDB文件，也就是说，创建RDB文件不能执行的过于频繁，否则会严重影响服务器的性能   每周做一次完全备份,每天做增量备份
2、可能丢失数据  突然断电或其他   备用电
```
...
wc -l 
   127.0.0.1:6379> BGREWRITEAOF  重写 
...
| 保存的间隔较长                                               | 保存的间隔默认为一秒钟                        |
| 数据还原速度快                                               | 数据还原速度一般，冗rong余命令多，还原速度慢  |
| 执行SAVE命令时会阻塞服务器，但手动或者自动触发的BGSAVE不会阻塞服务器 | 无论是平时还是进行AOF重写时，都不会阻塞服务器 |
|                                                              |                                               |

```python
# 用redis用来存储真正数据，每一条都不能丢失，都要用always，有的做缓存，有的保存真数据，我可以开多个redis服务，不同业务使用不同的持久化，新浪每个服务器上有4个redis服务，整个业务中有上千个redis服务，分不同的业务，每个持久化的级别都是不一样的。                                                     技术点业务逻辑
                                 以前是干什么的,业务逻辑搞明白,在找知识点,充满自信,自己说服了
```
...
```python
打开配置   sudo /etc/redis/redis.conf
# 设置密码 
