...
2、高性能的Key-Value的NoSQL数据库         非关系型数据库
...
2、MongoDB：键值对文档型数据库，值为JSON文档，基于磁盘，慢，存储数据类型单一  基于磁盘
3、Redis的诞生是为了解决什么问题？？ 基于内存
   # 解决硬盘IO带来的性能瓶颈
    场景:大量的并发出现时,mysql基于磁盘的,redis基于内存的,主要作用是解决大量并发问题.
```
...
# 客户端连接 
redis-cli -h IP地址 -p 6379 -a 密码    -HOST  -port  -a
redis-cli  默认连接本地
```
...
3、重启服务
  sudo /etc/init.d/redis-server restart[重点]
```
...
select number   use 库名
# 查看键
keys 表达式  # keys *   select * from 表名
# 数据类型
TYPE key
# 键是否存在
exists key
...
strlen key                       strlen
# 2.获取指定范围切片内容
getrange key start stop         getrange
# 3.从索引值开始，value替换原内容
setrange key index value        setrange
# 4.追加拼接value的值
...
```python
# 整数操作
INCRBY key 步长     incrby               
DECRBY key 步长
INCR key : +1操作  incr count
DECR key : -1操作
...
# 设置过期时间的两种方式  [过期时间]
# 方式一
1、set key value ex 3 [一种一条命令,绝不用两条]
# 方式二
1、set key value
2、expire key 5 # 秒
3、pexpire key 5 # 毫秒
...
# 增
1、从列表头部压入元素
	LPUSH key value1 value2                 [lpush]
2、从列表尾部压入元素
	RPUSH key value1 value2                 [rpush]
3、从列表src尾部弹出1个元素,压入到列表dst的头部
	RPOPLPUSH src dst         
4、在列表指定元素后/前插入元素
	LINSERT key after|before value newvalue
                                          linsert  key after|before
# 查
5、查看列表中元素
	LRANGE key start stop  [查]                      lrange key
...
8、从列表尾部弹出1个元素
	RPOP key    [尾]                                 rpop
9、列表头部,阻塞弹出,列表为空时阻塞
	BLPOP key timeout  [头部,阻塞]                 blpop key timeout
10、列表尾部,阻塞弹出,列表为空时阻塞
	BRPOP key timeout  [尾部,阻塞] block阻塞        brpop key timeout
  # 关于BLPOP 和 BRPOP                         弹出类型("key","laoqi")
  	1、如果弹出的列表不存在或者为空，就会阻塞
		2、超时时间设置为0，就是永久阻塞，直到有数据可以弹出
		3、如果多个客户端阻塞再同一个列表上，使用First In First Service原则，先到先服务
11、删除指定元素                   [删除元素]
	LREM key count value    [重点]
  count0>0：表示从头部开始向表尾搜索，移除与value相等的元素，数量为count
