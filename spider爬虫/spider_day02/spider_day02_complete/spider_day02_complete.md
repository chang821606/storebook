...
# 创建2个对象
db = pymysql.connect('localhost','root','123456','maoyandb',charset='utf8')                                                             重点
cursor = db.cursor()

# 执行SQL命令并提交到数据库执行
# execute()方法第二个参数为列表传参补位
ins = 'insert into filmtab values(%s,%s,%s)'
cursor.execute(ins,['霸王别姬','张国荣','1993'])   列表
db.commit()
...
#非关系型数据库,基于磁盘存储,存储以json形式



```python
import pymongo                                  mongodb          非关系型数据库
